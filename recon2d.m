    ...
    if strcmp(method,'manual') 
        ...
        if strcmp(method,'PCG-left')
            ...
            if strcmp(method,'PCG-right')
                ...
                y_bar=myCG(@preconR,b,zeros(size(b)),50,1e-5); 
...
function vec = fillmesh1d(totallength,lowbound,highbound)
% specify upper/lower bounds, fill in with the appropriate spacing
vec=lowbound:(highbound-lowbound)/(totallength-1):highbound;
end